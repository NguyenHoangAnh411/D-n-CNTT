...
class AuthService {
  ...
  Future<void> signUpWithPhone({
    ...
    try {
      ...
      if (!formattedPhoneNumber.startsWith('+')) {
        formattedPhoneNumber = '+84${formattedPhoneNumber.replaceFirst('0', '')}';
      }

      if (!RegExp(r'^\+84\d{9}$').hasMatch(formattedPhoneNumber)) {
        onError('Số điện thoại không hợp lệ! Vui lòng nhập số điện thoại của Việt Nam theo định dạng E.164.');
        return;
      }
  ...
  static Future<void> signUpWithEmail({
    ...
    if (!_isValidEmail(email)) {
      ...
      _showDialog(context, 'Email không hợp lệ! Vui lòng nhập email đúng định dạng.');
      return;
    }
    ...
    try {
      ...
      if (userCredential.user != null && !userCredential.user!.emailVerified) {
        ...
        _showDialog(context, 'Email xác thực đã được gửi. Vui lòng kiểm tra hộp thư của bạn!');
      } else {
